...
class Pyhpbase
  ...
  public:
    ...
    Pyhpbase (int64_t nside, const string &scheme)
      ...
      {
      planck_assert((scheme=="RING")||(scheme=="NEST"),
    ...
    a_i query_disc(const a_d &ptg, double radius) const
      {
      planck_assert((ptg.ndim()==1)&&(ptg.shape(0)==2),
...
a_d_c local_alm2map(const a_c_c &alm, int64_t lmax, int64_t mmax, int64_t nside)
  ...
  arr<xcomplex<double> > my_alm
    ...
    (const_cast<complex<double> *>(&ar[0])),ar.size());
  Alm<xcomplex<double>> Alm;
  Alm.Set(my_alm,lmax,mmax);
  a_d_c res(12*nside*nside);
  auto rr=res.mutable_unchecked<1>();
  arr<double> rmap(&rr[0],12*nside*nside);
